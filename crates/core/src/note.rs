...
impl NostrEvent for NostrNote {
    fn pubkey_str(&self) -> std::borrow::Cow<'_, str> {
        std::borrow::Cow::Borrowed(&self.pubkey)
    }
    ...
    fn content_str(&self) -> std::borrow::Cow<'_, str> {
        std::borrow::Cow::Borrowed(&self.content)
    }
    fn id_hex(&self) -> Option<std::borrow::Cow<'_, str>> {
        self.id.as_deref().map(std::borrow::Cow::Borrowed)
    }
