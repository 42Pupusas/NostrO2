...
impl JsObj {
    ...
    fn set(&self, k: &str, v: &JsValue) {
        Reflect::set(&self.0, &JsValue::from_str(k), v).unwrap();
    }

    fn err(msg: &str) -> JsValue {
        JsValue::from(js_sys::Error::new(msg))
    }
    fn get(&self, k: &str) -> Result<JsValue, JsValue> {
        Reflect::get(&self.0, &JsValue::from_str(k))
            .map_err(|_| Self::err(&format!("missing field: {k}")))
    }
    fn string(&self, k: &str) -> Result<String, JsValue> {
        self.get(k)?
            .as_string()
            .ok_or_else(|| Self::err(&format!("{k}: expected string")))
    }
    fn opt_string(&self, k: &str) -> Result<Option<String>, JsValue> {
        let v = self.get(k)?;
        Ok(if v.is_undefined() || v.is_null() {
            None
        } else {
            v.as_string()
        })
    }
    fn f64(&self, k: &str) -> Result<f64, JsValue> {
        self.get(k)?
            .as_f64()
            .ok_or_else(|| Self::err(&format!("{k}: expected number")))
    }

    fn into_inner(self) -> JsValue {
        self.0
    }
        ...
        C: AsRef<str> + 'a,
    {
        ...
        for row in rows {
            let inner = Array::new();
            for c in row {
                inner.push(&JsValue::from_str(c.as_ref()));
            }
            outer.push(&inner);
        }
        outer.into()
            ...
            for j in 0..inner.length() {
                cells.push(
                    inner
                        .get(j)
                        .as_string()
                        .ok_or_else(|| Self::err(&format!("tags[{i}][{j}]: expected string")))?,
                );
            }
    ...
    fn from_fields(
        ...
        obj.set("created_at", &JsValue::from_f64(created_at as f64));
        obj.set("kind", &JsValue::from_f64(f64::from(kind)));
        obj.set("tags", tags_js);
        obj.set("content", &JsValue::from_str(content));
        if let Some(id) = id {
            obj.set("id", &JsValue::from_str(id));
        }
        if let Some(sig) = sig {
            obj.set("sig", &JsValue::from_str(sig));
        }
        obj.into_inner()
    }
}
...
impl From<&NostrNote> for JsValue {
    fn from(note: &NostrNote) -> Self {
        JsObj::from_fields(
            ...
            note.id.as_deref(),
            note.sig.as_deref(),
            &JsObj::tags_to_js(note.tags.iter()),
        )
    }
}
...
impl From<&NostrNoteView<'_>> for JsValue {
    fn from(view: &NostrNoteView<'_>) -> Self {
        JsObj::from_fields(
            ...
            view.id.as_deref(),
            view.sig.as_deref(),
            &JsObj::tags_to_js(view.tags.iter()),
        )
    }
}
...
impl TryFrom<JsValue> for NostrNote {
    ...
    fn try_from(val: JsValue) -> Result<Self, Self::Error> {
        ...
        #[allow(clippy::cast_possible_truncation)]
        let created_at = obj.f64("created_at")? as i64;
        #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
        let kind = obj.f64("kind")? as u32;
        ...
        Ok(Self {
            ...
            tags,
            content,
            id: obj.opt_string("id")?,
            sig: obj.opt_string("sig")?,
        })
    }
}
...
impl TagRowJson for Array {
    ...
    fn write_json_row<W: json_bourne::JsonWrite + ?Sized>(&self, sink: &mut W) -> Result<(), W::Error> {
        ...
        for j in 0..self.length() {
            ...
            if let Some(cell) = self.get(j).as_string() {
...
impl NostrEvent for JsValue {
    fn pubkey_str(&self) -> Cow<'_, str> {
        Cow::Owned(
            Reflect::get(self, &JsValue::from_str("pubkey"))
                .ok()
    ...
    fn created_at(&self) -> i64 {
        Reflect::get(self, &JsValue::from_str("created_at"))
            .ok()
            .and_then(|v| v.as_f64())
            .map(|f| f as i64)
            .unwrap_or(0)
    ...
    fn kind(&self) -> u32 {
        Reflect::get(self, &JsValue::from_str("kind"))
            .ok()
            .and_then(|v| v.as_f64())
            .map(|f| f as u32)
            .unwrap_or(0)
    ...
    fn content_str(&self) -> Cow<'_, str> {
        Cow::Owned(
            Reflect::get(self, &JsValue::from_str("content"))
                .ok()
                .and_then(|v| v.as_string())
                .unwrap_or_default(),
        )
    ...
    fn id_hex(&self) -> Option<Cow<'_, str>> {
        Reflect::get(self, &JsValue::from_str("id"))
            .ok()
            .and_then(|v| v.as_string())
            .map(Cow::Owned)
    }
    fn sig_hex(&self) -> Option<Cow<'_, str>> {
        Reflect::get(self, &JsValue::from_str("sig"))
            .ok()
            .and_then(|v| v.as_string())
            .map(Cow::Owned)
    }
...
mod tests {
    ...
    fn view_to_js_round_trips_through_note() {
        ...
        assert_eq!(n.content, back.content);
        assert_eq!(n.tags.len(), back.tags.len());
    }
    #[wasm_bindgen_test]
    fn rejects_missing_pubkey() {
        let obj = JsObj::new();
        obj.set("created_at", &JsValue::from_f64(1.0));
        obj.set("kind", &JsValue::from_f64(1.0));
        obj.set("tags", &Array::new().into());
        obj.set("content", &JsValue::from_str("hi"));
        assert!(NostrNote::try_from(obj.into_inner()).is_err());
    }
    #[wasm_bindgen_test]
    fn rejects_wrong_type_kind() {
        let obj = JsObj::new();
        obj.set("pubkey", &JsValue::from_str("aa"));
        obj.set("created_at", &JsValue::from_f64(1.0));
        obj.set("kind", &JsValue::from_str("not a number"));
        obj.set("tags", &Array::new().into());
        obj.set("content", &JsValue::from_str("hi"));
        assert!(NostrNote::try_from(obj.into_inner()).is_err());
    }
