...
fn charset_rev(c: u8) -> Option<u8> {
    ...
    CHARSET.iter().position(|&ch| ch == c).map(|i| i as u8)
}
...
pub fn encode(hrp: &str, data: &[u8]) -> Result<String, Bech32Error> {
    ...
    let base32 = convert_bits(data, 8, 5, true);
    let checksum = create_checksum(hrp, &base32);
...
pub fn decode(s: &str) -> Result<(String, Vec<u8>), Bech32Error> {
    ...
    for c in data_part.bytes() {
        data5.push(charset_rev(c).ok_or(Bech32Error::InvalidChar(c as char))?);
    }
...
mod tests {
    ...
    fn round_trip_nsec() {
        ...
        let encoded = encode("nsec", &secret).unwrap();
    ...
    fn round_trip_npub() {
        ...
        let encoded = encode("npub", &pubkey).unwrap();
    ...
    fn round_trip_varied_bytes() {
        ...
        let encoded = encode("nsec", &data).unwrap();
    ...
    fn rejects_mixed_case() {
        let encoded = encode("nsec", &[0; 32]).unwrap();
        let mixed = encoded[..5].to_uppercase() + &encoded[5..];
        assert!(matches!(decode(&mixed), Err(Bech32Error::MixedCase)));
    }

    #[test]
    fn rejects_bad_checksum() {
        let mut encoded = encode("nsec", &[0; 32]).unwrap();
    ...
    fn rejects_empty_hrp() {
        assert!(matches!(encode("", &[0; 32]), Err(Bech32Error::InvalidHrp)));
    }

    #[test]
    fn accepts_uppercase_input() {
        let encoded = encode("nsec", &[0xff; 32]).unwrap();
