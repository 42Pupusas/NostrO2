...
pub enum Nip59Error {
    ...
    SerializationError(crate::json::JsonError),
    ParseError(String),
    SigningError,
}
...
pub trait Nip59: crate::nip_44::Nip44 + nostro2::NostrSigner {
    ...
    fn rumor(&self, giftwrap: &nostro2::NostrNote) -> Result<nostro2::NostrNote, Nip59Error> {
        ...
        if !giftwrap.verify() {
            return Err(Nip59Error::ParseError(
                "Giftwrap signature verification failed".into(),
            ));
        }
        let seal_note = self
            ...
            .map_err(|_| {
                Nip59Error::ParseError("Failed to parse NostrNote from giftwrap".into())
            })?;
        if !seal_note.verify() {
            return Err(Nip59Error::ParseError(
                "Seal note signature verification failed".into(),
            ));
        }
        let rumor_note: nostro2::NostrNote = self
            .nip_44_decrypt(&seal_note.content.clone(), &seal_note.pubkey)?
            .parse()
            .map_err(|_| Nip59Error::ParseError("Failed to parse NostrNote from seal".into()))?;
        if seal_note.pubkey != rumor_note.pubkey {
            return Err(Nip59Error::ParseError(
                "Seal note pubkey does not match rumor note pubkey".into(),
            ));
        }
        Ok(rumor_note)
    }
    ...
    fn seal(
        ...
        if rumor.created_at == 0 {
            rumor.created_at = Self::jittered_timestamp();
        }
        rumor
            .sign_with(self)
            .map_err(|_| Nip59Error::ParseError("Failed to sign NostrNote".into()))?;
        if !rumor.verify() {
            return Err(Nip59Error::SigningError);
        }
        rumor.sig.take();
        let mut seal = nostro2::NostrNote {
            content: crate::json::NipJson::to_string(rumor)
                .map_err(Nip59Error::SerializationError)?,
            kind: 13,
        ...
        Self: Sized,
    {
        ...
        let mut gw = nostro2::NostrNote {
            content: crate::json::NipJson::to_string(&sealed)
                .map_err(Nip59Error::SerializationError)?,
            kind: 1059,
        ...
        tk.nip44_encrypt_note(&mut gw, peer_pubkey)
            .map_err(|_| Nip59Error::ParseError("Failed to sign NostrNote".into()))?;
        gw.sign_with(&tk)
            .map_err(|_| Nip59Error::ParseError("Failed to sign NostrNote".into()))?;
        Ok(gw)
    ...
    fn replaceable_giftwrap(
        ...
        gw.tags.add_pubkey_tag(peer_pubkey, None);
        self.nip44_encrypt_note(&mut gw, peer_pubkey)
            .map_err(|_| Nip59Error::ParseError("Failed to sign NostrNote".into()))?;
        gw.sign_with(self)
            .map_err(|_| Nip59Error::ParseError("Failed to sign NostrNote".into()))?;
        Ok(gw)
    ...
    fn parameterized_giftwrap(
        ...
        gw.tags.add_pubkey_tag(peer_pubkey, None);
        gw.tags.add_parameter_tag(d_tag);
        self.nip44_encrypt_note(&mut gw, peer_pubkey)
            .map_err(|_| Nip59Error::ParseError("Failed to sign NostrNote".into()))?;
        gw.sign_with(self)
            .map_err(|_| Nip59Error::ParseError("Failed to sign NostrNote".into()))?;
...
mod tests {
    ...
    fn test_parameterized_rumor() {
        ...
        let mut n = mknote("secret rumor");
        let g = sender
            .parameterized_giftwrap(&mut n, &recv.public_key(), "test-d")
            .unwrap();
    ...
    fn test_replaceable_giftwrap_kind() {
        ...
        let recv = NipTester::generate();
        let mut s = sender
            .seal(&mut mknote("replaceable"), &recv.public_key())
            .unwrap();
        assert_eq!(
            sender
                .replaceable_giftwrap(&mut s, &recv.public_key())
                .unwrap()
                .kind,
            10059
        );
    ...
    fn test_ephemeral_giftwrap_kind() {
        ...
        let recv = NipTester::generate();
        let mut s = sender
            .seal(&mut mknote("ephemeral"), &recv.public_key())
            .unwrap();
        assert_eq!(
            sender
                .ephemeral_giftwrap(&mut s, &recv.public_key())
                .unwrap()
                .kind,
            20059
        );
    ...
    fn test_parameterized_giftwrap_tag_and_kind() {
        ...
        let recv = NipTester::generate();
        let mut s = sender
            .seal(&mut mknote("param"), &recv.public_key())
            .unwrap();
        let g = sender
            .parameterized_giftwrap(&mut s, &recv.public_key(), "test-d")
            .unwrap();
        assert_eq!(g.kind, 30059);
        assert_eq!(g.tags.first_parameter(), Some("test-d".into()));
    }
    fn dummy_json_err() -> crate::json::JsonError {
        crate::json::NipJson::parse_str::<i32>("!!!").unwrap_err()
    }
    #[test]
    fn error_display_covers_all_variants() {
        for err in &[
            Nip59Error::Nip44Error(crate::Nip44Error::SharedSecretError),
            Nip59Error::SerializationError(dummy_json_err()),
