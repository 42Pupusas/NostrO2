...
mod tests {
    use super::*;
    use crate::{Nip44, tests::NipTester};
    use nostro2::{NostrKeypair, NostrSigner};
    #[test]
    fn nip46_request() {
        let a = NipTester::generate();
        let b = NipTester::generate();
        let req = a
            .nip46_request(Nip46Method::Connect, vec!["test".into()], &b.public_key())
            .unwrap();
    ...
    fn nip46_response() {
        ...
        let r: Nip46Request = content.parse().unwrap();
        let resp = a
            .nip46_response(&r.id, "test".into(), None, &req.pubkey)
            .unwrap();
