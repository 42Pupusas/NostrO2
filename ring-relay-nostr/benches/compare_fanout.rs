...
impl FanoutHarness {
    ...
    fn iterate(&mut self) {
        ...
        // Publish synchronously over each pub's sink. We block on the tokio
        // runtime but the entire iterate() runs within rt.block_on so all
        // the awaits are serviceable.
        let pools = self.pub_pools.clone();
        let sinks = &mut self.pub_sinks;
        self.rt.block_on(async {
            for (sink, pool) in sinks.iter_mut().zip(pools.iter()) {
        ...
        // Spin on delivery count. This is pure user-space polling; the
        // subscriber tasks are on runtime workers and make progress
        // concurrently.
        ...
        loop {
            ...
            // Yield to the runtime briefly so sub tasks can make progress.
