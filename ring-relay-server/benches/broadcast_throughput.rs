    ...
    group.bench_function("ring_1x1", |b| {
        ...
        b.iter_custom(|iters| {
            ...
            for _ in 0..iters {
                ...
                println!("ring 1x1: {got}/{expected} deliveries in {total:.2?} ({rate:.0} msg/s)");
...
fn bench_ring_broadcast_sharded(c: &mut Criterion) {
    const NUM_BROADCASTS: usize = 10_000;
    ...
    group.bench_function(&format!("ring_{reader_shards}x{writer_shards}"), |b| {
        ...
        b.iter_custom(|iters| {
            ...
            for _ in 0..iters {
                ...
                let rate = got as f64 / total.as_secs_f64();
                println!(
                    "ring {reader_shards}x{writer_shards}: {got}/{expected} deliveries in {total:.2?} ({rate:.0} msg/s)"
                );
...
criterion_group!(benches, bench_ring_broadcast, bench_ring_broadcast_sharded, bench_tokio_broadcast);
