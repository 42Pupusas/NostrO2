...
use secp256k1::{SecretKey, KeyPair, XOnlyPublicKey, ecdh::shared_secret_point, PublicKey, Parity};
...
pub fn get_shared_point(private_keypair: KeyPair, public_key_string: String) -> Result<[u8; 32], UserError>{
    let hex_pk = hex::decode(public_key_string).map_err(|_| UserError::DecodingError)?;
    let x_only_public_key =
        XOnlyPublicKey::from_slice(hex_pk.as_slice()).map_err(|_| UserError::DecodingError)?;
